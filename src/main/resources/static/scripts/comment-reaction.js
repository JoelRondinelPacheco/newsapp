...
$likes.forEach(like => {
    like.addEventListener("click", async () => {
        ...
        let res = await commentReaction(likeURL)
        console.log(res)
        let icons = changeIcon(res, true, false)
        console.log(icons)
        like.innerHTML = icons[0]
        if (res === "Reaction changed") {
            const dislike = like.nextElementSibling;
            dislike.innerHTML = icons[1]
        }
...
$dislikes.forEach(dislike => {
    dislike.addEventListener("click", async () => {
        ...
        let res = await commentReaction(dislikeURL)
        let icons = changeIcon(res, false, false)
        console.log(res)
        console.log(icons)
        dislike.innerHTML = icons[0]
        if (res === "Reaction changed") {
            const like = dislike.previousElementSibling
            like.innerHTML = icons[1]
        }
    })
})
...
function changeIcon(responseText, isLike, isReport) {
    switch (responseText) {
